...
class EmbeddingSimilarityEvaluator:
    ...
    """
    def __init__(self, sentences1: List[str], sentences2: List[str], scores: List[float], batch_size: int = 16, main_similarity: int = None, name: str = '', show_progress_bar: bool = False, write_csv: bool = True):
        """
    ...
    def __call__(self, model, output_path: str = None, epoch: int = -1, steps: int = -1) -> float:
        ...
        elif self.main_similarity == EUCLIDEAN:
            return eval_spearman_euclidean
        elif self.main_similarity == MANHATTAN:
            return eval_spearman_manhattan
        elif self.main_similarity == DOT_PRODUCT:
